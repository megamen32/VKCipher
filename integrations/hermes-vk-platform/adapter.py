...
class VKAdapter(BasePlatformAdapter):
    ...
    def _outbound_chunks(self, chat_id: str, content: str) -> list[str]:
        formatted = self.format_message(content or "")
        if not self.vkencrypt.enabled:
            return self.truncate_message(formatted, self.max_message_length)
        if self.vkencrypt.has_session(chat_id) or self.vkencrypt.require_session:
            return self.vkencrypt.encrypted_chunks(chat_id, formatted, self.max_message_length)
        return self.truncate_message(formatted, self.max_message_length)

    def _protect_caption(self, chat_id: str, caption: Optional[str]) -> str:
        value = caption or ""
        if not value or not self.vkencrypt.enabled:
            return value
        return self.vkencrypt.encrypt_outbound(chat_id, value)

    def _media_send_blocked(self) -> Optional[SendResult]:
        if self.vkencrypt.enabled and not self.vkencrypt.allow_unencrypted_media:
            return SendResult(
                success=False,
                error="VKEncrypt text mode refuses to send unencrypted media; set VK_ENCRYPT_ALLOW_UNENCRYPTED_MEDIA=true explicitly",
                retryable=False,
            )
        return None
    ...
    async def _handle_update(self, update: dict[str, Any]) -> None:
        ...
        if self.vkencrypt.enabled and attachments and not self.vkencrypt.allow_unencrypted_media:
            logger.info("VKEncrypt: ignored unencrypted media peer=%s", peer_id)
            attachments = []
    ...
    async def _send_attachment(self, chat_id: str, attachment: str, caption: Optional[str] = None) -> SendResult:
        blocked = self._media_send_blocked()
        if blocked:
            return blocked
        try:
            protected_caption = self._protect_caption(chat_id, caption)
            async def op():
                return await self._vk_method(
                    "messages.send",
                    {
                        "peer_ids": str(chat_id),
                        "message": protected_caption,
    ...
    async def send(
        ...
        chunks = self._outbound_chunks(chat_id, content)
    ...
    async def edit_message(
        ...
        **kwargs: Any,
        ...
        try:
            if self.vkencrypt.enabled:
                edit_message = self._outbound_chunks(chat_id, content)[0] if content else ""
            else:
                edit_message = self.format_message(content or "")
            edit_params = {
                "peer_id": str(chat_id),
