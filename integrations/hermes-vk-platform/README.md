...
For the VKEncrypt-integrated copy in this repository, install the vendored
directory instead of the upstream plugin:

```bash
rm -rf ~/.hermes/plugins/vk
cp -R integrations/hermes-vk-platform ~/.hermes/plugins/vk
hermes gateway restart
```
...
```bash
printf '%s\n' 'the same VKEncrypt seed' > ~/.hermes/vkencrypt-vk.seed
...
```dotenv
VK_ENCRYPT_SEED_FILE=~/.hermes/vkencrypt-vk.seed
```

Encrypted text mode is fail-closed by default:
...
plaintext fallback. `VK_ENCRYPT_ALLOW_UNENCRYPTED_MEDIA=true` permits media
uploads without encryption and should not be used for a private encrypted chat.
