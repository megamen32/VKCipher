...
if (!/^\d+(\.\d+){1,3}$/u.test(version)) {
    throw new Error(`Chrome manifest version is invalid: ${version}`);
}

const outputRoot = path.join(ROOT, 'dist', 'chrome');
const packageRoot = path.join(outputRoot, 'package');
const zipPath = path.join(outputRoot, `vkencrypt-chrome-v${version}.zip`);
...
console.log(`Built Chrome package: ${zipPath}`);
