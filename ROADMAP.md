...
- Hermes VK integration discovery and adapter proposal.
  ...
  Status: implementation and server-100 operations map complete for vendored plugin, text crypto, and opt-in Russian-word transport; live VK short/multipart E2E passes on server-100, media encryption remains queued.
...
- Hermes server-100 operations map: sanitized links to config, services, plugins/adapters, runtime, logs, sessions, caches, backups, and source are maintained in `docs/hermes-server-100.md` and deployed to `~/.hermes/ops/README.md`.
